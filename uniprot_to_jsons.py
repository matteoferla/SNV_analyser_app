...
class UniprotReader:
    ...
    def convert(cls, uniprot_master_file, first_n_protein=0):
        ...
        if not os.path.isfile(uniprot_master_file):
            ...
            raise FileNotFoundError('Please download uniprot_sprot.xml from FTP Uniprot site')
        count=0
        namedex = {}
        seqdex = {}
        genedex = {}
        for entry in cls(uniprot_master_file).iter_human():
            ...
            ## save
            prot.write(os.path.join('data','uniprot',chosen_name+'_uniprot.xml'))
            # prot.
        ## cleanup
        for k in ('\n      ', '\n     ', '\n    ', '\n   ', '', '\n', ' '):
            if k in namedex:
                del namedex[k]
        json.dump(namedex, open(os.path.join('data', 'human_prot_namedex.json'), 'w'))
        json.dump(genedex, open(os.path.join('data', 'human_prot_genedex.json'), 'w'))
        json.dump(seqdex, open(os.path.join('data', 'human_prot_seqdex.json'), 'w'))


if __name__ == '__main__':
    UniprotReader.convert(uniprot_master_file = os.path.join('data', 'references', 'uniprot_sprot.xml'), first_n_protein=0)
